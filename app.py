...
# نمودار میله‌ای به تفکیک پیمانکار
st.subheader("📊 نمودار میله‌ای پیشرفت به تفکیک پیمانکار")
contractor_progress = (
...
fig_bar.update_layout(title="مقایسه پیشرفت واقعی و برنامه‌ای پیمانکاران")
st.plotly_chart(fig_bar, use_container_width=True)

# نمودار روند زمانی پیشرفت تجمعی
st.subheader("📈 نمودار روند تجمعی پیشرفت")
...
fig_trend.update_layout(title="روند تجمعی پیشرفت پروژه")
st.plotly_chart(fig_trend, use_container_width=True)

# نمودار S Curve (PV/EV/AC)
st.subheader("📈 نمودار S Curve (PV / EV / AC)")
timeline_df["PV"] = timeline_df["Planned Progress I"].cumsum()
timeline_df["EV"] = timeline_df["Actual Progress I"].cumsum()
timeline_df["AC"] = filtered_df["Actual Cost"].fillna(0).cumsum()
...
fig_s_curve.update_layout(title="نمودار S Curve (EV / PV / AC)")
